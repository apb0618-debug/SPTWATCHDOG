...
from PySide6.QtCore import Qt, QTimer, QPropertyAnimation, Signal, QSignalBlocker
...
THEMES = {
    "Default": {
        ...
        "TEXT": "#F3EDE6",
        "MUTED": "#AE9F92",
        "FOOTER_ACCENT": "#E4B58C",
        "SECTION_ACCENT": "#E0A06A",
        "LIST_ITEM_BORDER": "#241711",
        "LIST_ITEM_SELECTED_BG": "#2A1D15",
        "SECONDARY_BUTTON_BG": "#211915",
        "SECONDARY_BUTTON_HOVER": "#2A201B",
        "LOG_BG": "#15110F",
        "LOG_BORDER": "#261812",
        "LOG_TEXT": "#E6DDD3",
        "UPDATE_POPUP_BG": "rgba(18, 15, 13, 0.96)",
    },
    "Blue Steel": {
        "ACCENT": "#4F83C2",
        "ACCENT_HOVER": "#6A9BDA",
        ...
        "TEXT": "#EAF1F7",
        "MUTED": "#9FAFBE",
        "FOOTER_ACCENT": "#9DC3EA",
        "SECTION_ACCENT": "#87B3E1",
        "LIST_ITEM_BORDER": "#1E2B38",
        "LIST_ITEM_SELECTED_BG": "#223242",
        "SECONDARY_BUTTON_BG": "#172028",
        "SECONDARY_BUTTON_HOVER": "#1E2A35",
        "LOG_BG": "#10161C",
        "LOG_BORDER": "#203142",
        "LOG_TEXT": "#E3ECF5",
        "UPDATE_POPUP_BG": "rgba(12, 18, 24, 0.96)",
    },
    "Night Tide": {
        "ACCENT": "#2F6FD8",
        "ACCENT_HOVER": "#4D89EB",
        ...
        "TEXT": "#EDF4FF",
        "MUTED": "#90A3BB",
        "FOOTER_ACCENT": "#8EB8FF",
        "SECTION_ACCENT": "#74A4F0",
        "LIST_ITEM_BORDER": "#152233",
        "LIST_ITEM_SELECTED_BG": "#1B2A3D",
        "SECONDARY_BUTTON_BG": "#101722",
        "SECONDARY_BUTTON_HOVER": "#152132",
        "LOG_BG": "#0D131D",
        "LOG_BORDER": "#1B2B43",
        "LOG_TEXT": "#E6F0FF",
        "UPDATE_POPUP_BG": "rgba(8, 13, 21, 0.96)",
    },
    "Gunmetal": {
        "ACCENT": "#7A8896",
        "ACCENT_HOVER": "#96A3AF",
        ...
        "TEXT": "#ECEFF3",
        "MUTED": "#A0A8B2",
        "FOOTER_ACCENT": "#C0C8D0",
        "SECTION_ACCENT": "#AEB8C2",
        "LIST_ITEM_BORDER": "#22272D",
        "LIST_ITEM_SELECTED_BG": "#2B3138",
        "SECONDARY_BUTTON_BG": "#15181C",
        "SECONDARY_BUTTON_HOVER": "#1B1F24",
        "LOG_BG": "#111418",
        "LOG_BORDER": "#242A31",
        "LOG_TEXT": "#E8ECEF",
        "UPDATE_POPUP_BG": "rgba(17, 19, 22, 0.96)",
    },
    "Crimson Tide": {
        "ACCENT": "#B33A3A",
        "ACCENT_HOVER": "#CF5151",
        ...
        "TEXT": "#F7ECEC",
        "MUTED": "#B79D9D",
        "FOOTER_ACCENT": "#E0A0A0",
        "SECTION_ACCENT": "#D88686",
        "LIST_ITEM_BORDER": "#301818",
        "LIST_ITEM_SELECTED_BG": "#371F1F",
        "SECONDARY_BUTTON_BG": "#1C1111",
        "SECONDARY_BUTTON_HOVER": "#261616",
        "LOG_BG": "#140E0E",
        "LOG_BORDER": "#2C1818",
        "LOG_TEXT": "#F4E8E8",
        "UPDATE_POPUP_BG": "rgba(20, 11, 11, 0.96)",
    },
    "Neon": {
        "ACCENT": "#39D353",
        "ACCENT_HOVER": "#59E272",
        ...
        "TEXT": "#EDF9EE",
        "MUTED": "#99B39D",
        "FOOTER_ACCENT": "#94F5A4",
        "SECTION_ACCENT": "#7CEB90",
        "LIST_ITEM_BORDER": "#1A2E1C",
        "LIST_ITEM_SELECTED_BG": "#1F3222",
        "SECONDARY_BUTTON_BG": "#101610",
        "SECONDARY_BUTTON_HOVER": "#172017",
        "LOG_BG": "#0E140F",
        "LOG_BORDER": "#1E3522",
        "LOG_TEXT": "#E9F8EB",
        "UPDATE_POPUP_BG": "rgba(11, 17, 11, 0.96)",
    },
}
...
def normalize_theme_name(theme_name: str) -> str:
    ...
    candidate = theme_name.strip()
    return candidate if candidate in THEMES else DEFAULT_THEME_NAME


def apply_theme_palette(theme_name: str) -> str:
    normalized = normalize_theme_name(theme_name)
    palette = THEMES[normalized]
...
ICON_CANDIDATES = ("watchdog.ico", "watchdog.png")


def get_app_icon() -> QIcon:
    for icon_name in ICON_CANDIDATES:
        try:
            candidate = resource_path(icon_name)
...
class ManagedProcess:
    ...
    def prepare_for_stop(self):
        with self.lock:
            ...
            self.last_auto_restart_attempt = time.monotonic()
...
class WatchdogSuiteWindow(QMainWindow):
    ...
    def _coerce_loaded_config(self, raw):
        ...
        try:
            ...
            clean["theme_name"] = normalize_theme_name(clean.get("theme_name", DEFAULT_THEME_NAME))
            return AppConfig(**clean)
        except Exception as exc:
            self.log(f"[config] invalid config data detected; using defaults. Details: {exc}")
            return AppConfig()

    def _coerce_addition_entries(self, raw):
        clean_entries = []
        if not isinstance(raw, list):
            self.log("[additions] invalid additions data detected; using empty list.")
            return clean_entries
        for idx, entry in enumerate(raw):
            if not isinstance(entry, dict):
                self.log(f"[additions] skipped malformed entry at index {idx}.")
                continue
            try:
                normalized = asdict(AdditionEntry(
                    created_at=str(entry.get("created_at") or datetime.now().isoformat(timespec="seconds")),
                    title=str(entry.get("title") or "Untitled Entry"),
                    entry_date=str(entry.get("entry_date") or datetime.now().strftime("%m/%d/%y")),
                    addition_type=str(entry.get("addition_type") or ADDITION_TYPES[0]),
                    additions=str(entry.get("additions") or ""),
                    notes=str(entry.get("notes") or ""),
                ))
                if normalized["addition_type"] not in ADDITION_TYPES:
                    normalized["addition_type"] = ADDITION_TYPES[0]
                clean_entries.append(normalized)
            except Exception as exc:
                self.log(f"[additions] skipped malformed entry at index {idx}: {exc}")
        return clean_entries

    def _coerce_issue_entries(self, raw):
        clean_entries = []
        if not isinstance(raw, list):
            self.log("[issues] invalid issue data detected; using empty list.")
            return clean_entries
        for idx, entry in enumerate(raw):
            if not isinstance(entry, dict):
                self.log(f"[issues] skipped malformed entry at index {idx}.")
                continue
            try:
                normalized = asdict(IssueEntry(
                    created_at=str(entry.get("created_at") or datetime.now().isoformat(timespec="seconds")),
                    title=str(entry.get("title") or "Untitled Issue"),
                    issue_date=str(entry.get("issue_date") or datetime.now().strftime("%m/%d/%y")),
                    issue_type=str(entry.get("issue_type") or ISSUE_TYPES[0]),
                    description=str(entry.get("description") or ""),
                    recent_items=str(entry.get("recent_items") or ""),
                    fix_notes=str(entry.get("fix_notes") or ""),
                ))
                if normalized["issue_type"] not in ISSUE_TYPES:
                    normalized["issue_type"] = ISSUE_TYPES[0]
                clean_entries.append(normalized)
            except Exception as exc:
                self.log(f"[issues] skipped malformed entry at index {idx}: {exc}")
        return clean_entries

    def load_additions(self):
        self.additions = self._coerce_addition_entries(self.load_json(ADDITIONS_FILE, []))

    def save_additions(self):
        self.save_json(ADDITIONS_FILE, self.additions)

    def load_issues(self):
        self.issues = self._coerce_issue_entries(self.load_json(ISSUES_FILE, []))
    ...
    def send_discord_webhook_async(self, message: str, force=False):
        ...
        threading.Thread(target=worker, daemon=True, name="DiscordWebhookWorker").start()
    ...
    def _build_settings_page(self):
        ...
        appearance_grid = QGridLayout(); appearance_grid.setHorizontalSpacing(10); appearance_grid.setVerticalSpacing(10)
        self.theme_combo = QComboBox(); self.theme_combo.addItems(THEME_NAMES)
        self.theme_combo.currentTextChanged.connect(self.preview_theme_selection)
        appearance_grid.addWidget(self._label("Color Scheme", "fieldLabel"), 0, 0)
        appearance_grid.addWidget(self.theme_combo, 0, 1)
        appearance.body.addLayout(appearance_grid)
        appearance_note = self._label("Cosmetic only. Status colors remain fixed for clarity.", "helperText")
        appearance_note.setWordWrap(True)
        appearance.body.addWidget(appearance_note)
    ...
    def _build_tray(self):
        ...
        if not self.tray_available:
            self.log("[tray] system tray unavailable on this system.")
    ...
    def _ensure_tray_ready(self) -> bool:
        ...
        try:
            ...
            self.log(f"[tray] failed to show tray icon: {exc}")
            return False

        if not self.tray_icon.isVisible():
            self.log("[tray] tray icon failed to become visible. Minimize-to-tray cancelled.")
            return False
        return True

    def _apply_theme(self, theme_name: str, sync_combo=False):
        normalized = apply_theme_palette(theme_name)
        if sync_combo and hasattr(self, "theme_combo"):
            blocker = QSignalBlocker(self.theme_combo)
            self.theme_combo.setCurrentText(normalized)
            del blocker
        self._apply_styles()
        return normalized

    def preview_theme_selection(self, theme_name: str):
    ...
    def _apply_styles(self):
        self.setStyleSheet(f"""
            ...
            #footerPrimary {{ color: {FOOTER_ACCENT}; font-size: 12px; font-weight: 700; background: transparent; }}
            ...
            #sectionTitle {{ color: {SECTION_ACCENT}; font-size: 12px; font-weight: 800; letter-spacing: 0.8px; background: transparent; }}
            ...
            QComboBox QAbstractItemView {{ background: {PANEL_ALT}; border: 1px solid {BORDER}; color: {TEXT}; selection-background-color: {ACCENT}; }}
            ...
            QListWidget::item {{ padding: 8px; border-bottom: 1px solid {LIST_ITEM_BORDER}; background: transparent; }}
            QListWidget::item:selected {{ background: {LIST_ITEM_SELECTED_BG}; border: 1px solid {ACCENT}; }}
            ...
            #secondaryButton {{ background: {SECONDARY_BUTTON_BG}; border: 1px solid {BORDER}; color: {TEXT}; }}
            #secondaryButton:hover {{ background: {SECONDARY_BUTTON_HOVER}; border-color: {ACCENT}; }}
            #logBox {{ background: {LOG_BG}; border: 1px solid {LOG_BORDER}; border-radius: 12px; font-family: 'Consolas'; font-size: 12px; color: {LOG_TEXT}; }}
            #updatePopup {{ background: {UPDATE_POPUP_BG}; border: 1px solid {ACCENT}; border-radius: 14px; }}
    ...
    def snapshot_config_from_ui(self):
        return AppConfig(
            ...
            theme_name=normalize_theme_name(self.theme_combo.currentText()) if hasattr(self, "theme_combo") else DEFAULT_THEME_NAME,
        )
    ...
    def apply_config_to_ui(self):
        ...
        self._apply_theme(self.config.theme_name, sync_combo=True)
    ...
    def start_server(self):
        if not self.collect_config_from_ui():
            return
        threading.Thread(target=self._start_server_with_minimize, daemon=True, name="StartServerWorker").start()

    def start_headless(self):
        if not self.collect_config_from_ui():
            return
        threading.Thread(target=self._start_headless_with_delay, daemon=True, name="StartHeadlessWorker").start()

    def start_both(self):
        if not self.collect_config_from_ui():
            return
        threading.Thread(target=self._start_both_impl, daemon=True, name="StartBothWorker").start()

    def stop_server(self):
        self.server_proc.prepare_for_stop()
        threading.Thread(target=self.server_proc.stop, daemon=True, name="StopServerWorker").start()

    def stop_headless(self):
        self._next_headless_launch_id()
        self.headless_proc.prepare_for_stop()
        threading.Thread(target=self.headless_proc.stop, daemon=True, name="StopHeadlessWorker").start()

    def stop_both(self):
        self.server_proc.prepare_for_stop()
        self.headless_proc.prepare_for_stop()
        threading.Thread(target=self._stop_both_impl, daemon=True, name="StopBothWorker").start()

    def restart_both(self):
        if not self.collect_config_from_ui():
            return
        threading.Thread(target=self._restart_both_impl, daemon=True, name="RestartBothWorker").start()

    def _start_both_impl(self):
        self._start_server_with_minimize()
        threading.Thread(target=self._start_headless_with_delay, daemon=True, name="DelayedHeadlessWorker").start()
    ...
    def _restart_both_impl(self):
        ...
        try:
            ...
            self._start_server_with_minimize()
            self._next_headless_launch_id()
            threading.Thread(target=self._start_headless_with_delay, daemon=True, name="StartBothHeadlessWorker").start()
    ...
    def start_monitor(self):
        ...
        self.monitor_thread = threading.Thread(target=self.monitor_loop, daemon=True, name="WatchdogMonitorLoop")
        ...
        threading.Thread(target=self._start_both_impl, daemon=True, name="MonitorStartProcessesWorker").start()
    ...
    def _auto_minimize_to_tray_if_running(self):
        if self.config.minimize_to_tray and self.monitor_running:
            if not self.hide_to_tray(interactive=False):
                self.log("[tray] auto-minimize skipped because tray icon was not ready.")
    ...
    def stop_monitor(self):
        ...
        threading.Thread(target=self._stop_both_impl, daemon=True, name="MonitorStopWorker").start()
        self.update_status_ui()

    def monitor_loop(self):
        while not self.monitor_stop_event.is_set():
            try:
                if self.monitor_stop_event.is_set() or self._shutting_down:
                    break
                    ...
                    if self.config.auto_restart_on_crash:
                        ...
                        if self.headless_proc.process is not None and not self.headless_proc.is_running() and not self.headless_proc.stop_requested:
                            if self.headless_proc.allow_auto_restart():
                                ...
                                threading.Thread(target=self._start_headless_with_delay, daemon=True, name="CrashRecoveryHeadlessWorker").start()
                    if self.next_restart_time and datetime.now() >= self.next_restart_time:
                        self.log("[monitor] scheduled restart interval reached.")
                        threading.Thread(target=self._restart_both_impl, daemon=True, name="ScheduledRestartWorker").start()
                self.status_refresh_requested.emit()
            except Exception as e:
    ...
    def hide_to_tray(self, interactive: bool = True):
        if not self.config.minimize_to_tray:
            return False
        if not self._ensure_tray_ready():
            if interactive:
                QMessageBox.warning(self, APP_NAME, "Minimize to tray is enabled, but the tray icon could not be shown. WATCHDOG will stay visible so you do not lose access to the app.")
            else:
                self.log("[tray] tray icon unavailable; staying visible.")
            return False
        self.hide()
    ...
    def show_from_tray(self):
        ...
        if self.tray_icon:
            self.tray_icon.hide()
        self.log("[tray] restored from tray.")
    ...
    def closeEvent(self, event):
        ...
        if self.config.minimize_to_tray:
            if self.hide_to_tray(interactive=True):
                event.ignore()
                return
            self.log("[tray] close-to-tray fallback failed; proceeding with full app exit.")
        self._shutting_down = True
        self.monitor_stop_event.set()
        self.monitor_running = False
        self._next_headless_launch_id()
