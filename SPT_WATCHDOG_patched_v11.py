...
APP_DATA_DIR_NAME = "SPT Watchdog"
...
class WatchdogApp:
    ...
    def _build_ui(self):
        ...
        self._entry_row(paths_frame, 2, "Server Working Dir", self.server_workdir_var)
        self._entry_row(paths_frame, 3, "Headless Working Dir", self.headless_workdir_var)
        ...
        discord_frame = ttk.LabelFrame(main, text="Discord Notifications", padding=10)
        discord_frame.pack(fill="x", pady=(0, 10))

        ttk.Checkbutton(discord_frame, text="Enable Discord webhook notifications", variable=self.discord_enabled_var).grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 8))
        self._entry_row(discord_frame, 1, "Webhook URL", self.discord_webhook_var)
        ttk.Button(discord_frame, text="Test Webhook", command=self.test_discord_webhook).grid(row=2, column=1, sticky="w", padx=6, pady=(8, 0))
        ttk.Label(discord_frame, text="Paste the full Discord webhook URL. It stays local on this machine.").grid(row=2, column=0, columnspan=3, sticky="w", pady=(8, 0))
