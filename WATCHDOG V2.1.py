...
def _find_windows_for_pid(pid: int):
    ...
    _user32.EnumWindows(enum_cb(callback), 0)
    return windows


def minimize_windows_for_pid(pid: int, log_func=None, label: str = "Process", attempts: int = 18, delay: float = 0.75):
...
class ManagedProcess:
        ...
        with self.lock:
            ...
            try:
                ...
                if capture_output and self.process.stdout is not None:
                    self.reader_thread = threading.Thread(target=self._stream_output, args=(self.process,), daemon=True)
    ...
    def stop(self, timeout: int = 15) -> bool:
        with self.lock:
            ...
            try:
                if os.name == "nt":
                    result = subprocess.run(["taskkill", "/PID", str(pid), "/T", "/F"], capture_output=True, text=True, timeout=timeout, check=False)
...
class WatchdogSuiteWindow(QMainWindow):
        ...
        for candidate in get_legacy_config_candidates():
            try:
                if candidate.exists() and candidate.resolve() != CONFIG_FILE.resolve():
                    CONFIG_FILE.write_text(candidate.read_text(encoding="utf-8"), encoding="utf-8")
    ...
    def load_config(self):
        if CONFIG_FILE.exists():
            try:
                self.config = AppConfig(**json.loads(CONFIG_FILE.read_text(encoding="utf-8")))
            except Exception:
                self.config = AppConfig()
        else:
            self.config = AppConfig()
    ...
    def _start_headless_with_delay(self):
        ...
        if launch_id != self.pending_headless_start_id:
            ...
            return
        self.headless_proc.start(self.config.headless_path, self.config.headless_workdir)
        if self.config.hide_server_console and self.headless_proc.process is not None:
